...
crimeReg <- read.csv(file="crimeReg(1).csv", header=TRUE, row.names=1)
colnames(crimeReg)
...
#lasso.mad <-glmet(predictors, targetors,alpha=weight or the error penaltyl(usually 1))
...
lacbind(lambda=round(cv.out$lambda,1),
  cvm=signif(cv.out$cvm,7),
  cvsd=signif(cv.out$cvsd,5))
#based on the plot the bars = CV Std Diviation, round red - Cross Validation mean, 
#the numbers 79-1 are non zero selected variables and ----lines are 1st one MSE and second ---line
# is the MSE with phase 1 of 1st MSE
...
# of 123,389.
#Notice, the values might be different once you run the code due to randomization.
...
#signif(lasso.coef,3) - Signif() rounds of the decimal to the nearest variables
#rounds() - rounds to integer format
#sum(abs(lasso.coef)>0) - gives the number of non zero variables in the lasso model
